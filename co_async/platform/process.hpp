...
struct ProcessBuilder {
    ...
    ProcessBuilder &open(int fd, const FileHandle &file) {
    ...
    ProcessBuilder &pipe_out(int fd, OwningStream &stream) {
        ...
        /* Redirect child process's fd (e.g., STDOUT_FILENO) to pipe's write end
         * (p[1]). Child process output will be written here. */
        open(fd, FileHandle(p[1]));
        /* Create an OwningStream using the pipe's read end (p[0]). The parent
         * will read child output from this stream. */
    ...
    ProcessBuilder &pipe_in(int fd, OwningStream &stream) {
        int p[2];
        throwingErrorErrno(pipe2(p, 0));
        /* Redirect child process's fd (e.g., STDIN_FILENO) to pipe's read end
         * (p[0]). Child process will read input from here. */
        open(fd, FileHandle(p[0]));
        /* Create an OwningStream using the pipe's write end (p[1]). The parent
         * will write child input to this stream. */
    ...
    /* Set program path: mPath. isAbsolute flag indicates if path is absolute.
     */
    ...
    /* Add a command-line argument to mArgvStore. */
    ProcessBuilder &arg(std::string_view arg) {
        mArgvStore.emplace_back(arg);
        return *this;
    }

    /* If inherit is true, copy all environment variables from parent's current
     * environment (extern char **environ) to mEnvpStore. */
...
private:
    ...
    bool mAbsolutePath; /* Indicates if mPath is an absolute path, determining
                           whether to use posix_spawn or posix_spawnp. */
    bool mEnvInherited; /* Whether parent process's environment variables have
                           been inherited. */
    std::string mPath;  /* Path to program to execute. */
    std::vector<std::string>
        mArgvStore;     /* Stores child process command-line arguments. */
    std::vector<std::string>
        mEnvpStore;     /* Stores child process environment variables. */
