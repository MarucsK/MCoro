...
struct [[nodiscard]] FileHandle {
    ...
    FileHandle(FileHandle &&other) noexcept : mFileNo(other.releaseFile()) {}

    FileHandle &operator=(FileHandle &&other) noexcept {
        std::swap(mFileNo, other.mFileNo);
        return *this;
    }
...
struct FileStat {
    ...
    accessed_time() const { // Last access time.
        return statTimestampToTimePoint(mStatx.stx_atime);
    }

    std::chrono::system_clock::time_point
    attribute_changed_time() const { // Last attribute change time.
        return statTimestampToTimePoint(mStatx.stx_ctime);
    }

    std::chrono::system_clock::time_point
    created_time() const { // Creation time.
        return statTimestampToTimePoint(mStatx.stx_btime);
    }

    std::chrono::system_clock::time_point
    modified_time() const { // Last modification time.
...
private:
    ...
    statTimestampToTimePoint(const struct statx_timestamp &time) {
...
inline std::filesystem::path make_path(std::string_view path) {
    return std::filesystem::path(
        reinterpret_cast<const char8_t *>(std::string(path).c_str()));
}
...
inline Task<Expected<FileHandle>> fs_open(std::filesystem::path path,
    ...
    int fd = co_await expectError(co_await UringOp().prep_openat(
                 ...
                 .or_else(std::errc::bad_file_descriptor, [&] {
                     return expectError(open(path.c_str(), oflags, access));
                 });
    FileHandle file(fd);
    co_return file;
}
...
inline Task<Expected<FileHandle>> fs_openat(FileHandle dir,
    ...
    int oflags = static_cast<int>(mode);
    int fd =
        co_await expectError(co_await UringOp().prep_openat(
                                 dir.fileNo(), path.c_str(), oflags, access))
            .or_else(std::errc::bad_file_descriptor, [&] {
                return expectError(
                    openat(dir.fileNo(), path.c_str(), oflags, access));
            });
    FileHandle file(fd);
    co_return file;
}
...
inline Task<Expected<>> fs_mkdir(std::filesystem::path path,
                                 mode_t access = 0755) {
...
inline Task<Expected<>> fs_link(std::filesystem::path oldpath,
                                std::filesystem::path newpath) {
    co_await expectError(co_await UringOp().prep_linkat(
        AT_FDCWD, oldpath.c_str(), AT_FDCWD, newpath.c_str(), 0));
    co_return {};
}

inline Task<Expected<>> fs_symlink(std::filesystem::path target,
...
inline Task<Expected<>> fs_rmdir(std::filesystem::path path) {
    co_await expectError(
        co_await UringOp().prep_unlinkat(AT_FDCWD, path.c_str(), AT_REMOVEDIR));
    co_return {};
}

inline Task<Expected<FileStat>>
fs_stat(std::filesystem::path path,
        unsigned int mask = STATX_BASIC_STATS | STATX_BTIME, int flags = 0) {
    FileStat ret;
    co_await expectError(co_await UringOp().prep_statx(AT_FDCWD, path.c_str(),
                                                       flags, mask,
                                                       ret.getNativeStatx()))
        .or_else(std::errc::bad_file_descriptor, [&] {
            return expectError(statx(AT_FDCWD, path.c_str(), flags, mask,
                                     ret.getNativeStatx()));
        });
    co_return ret;
}
        ...
        co_await expectError(
            ...
            .or_else(std::errc::invalid_argument, [&] {
                if (offset == static_cast<std::uint64_t>(-1)) {
                    return expectError(static_cast<int>(
                        read(file.fileNo(), buffer.data(), buffer.size())));
                } else {
                    return expectError(static_cast<int>(
                        pread64(file.fileNo(), buffer.data(), buffer.size(),
...
inline Task<Expected<std::size_t>>
fs_write(FileHandle &file, std::span<const char> buffer,
         std::uint64_t offset = static_cast<std::uint64_t>(-1)) {
    co_return static_cast<std::size_t>(
        co_await expectError(
            ...
            .or_else(std::errc::invalid_argument, [&] {
                if (offset == static_cast<std::uint64_t>(-1)) {
                    ...
                    return expectError(static_cast<int>(
                        pwrite64(file.fileNo(), buffer.data(), buffer.size(),
                                 static_cast<__off64_t>(offset))));
                }
            }));
}
...
inline Task<Expected<std::size_t>>
fs_write(FileHandle &file, std::span<const char> buffer, CancelToken cancel,
         std::uint64_t offset = static_cast<std::uint64_t>(-1)) {
    co_return static_cast<std::size_t>(
        co_await expectError(co_await UringOp()
