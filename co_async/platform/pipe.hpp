...
inline Task<Expected<FSPipeHandlePair>> fs_pipe() {
    ...
    co_await expectError(
        res); // Short-circuits on error; otherwise, res is ignored.
    co_return FSPipeHandlePair{FileHandle(p[0]), FileHandle(p[1])};
}

/* Asynchronously send file content to sock using splice for zero-copy
 * transfer. */
...
/* Asynchronously receive data from sock to file. */
