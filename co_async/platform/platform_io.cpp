...
std::size_t
PlatformIOContext::addBuffers(std::span<const std::span<char>> bufs) {
...
void PlatformIOContext::reserveFiles(std::size_t nfiles) {
    ...
    throwingError(io_uring_register_files_update_tag(
        &mRing, 0, mFiles.get(), tags.data(),
        static_cast<unsigned int>(mNumFiles)));
}

std::size_t PlatformIOContext::addFiles(std::span<const int> files) {
