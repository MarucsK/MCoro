...
private:
    inline Stamp offsetRead(Stamp s) const { // get read index
        return s >> Shift;
    }

    inline Stamp offsetWrite(Stamp s) const { // get write index
        return s & (kSize - 1);
    }
    ...
    const std::unique_ptr<T[]> mHead = std::make_unique<T[]>(kSize);
