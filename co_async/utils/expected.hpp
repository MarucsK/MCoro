...
concept WeaklyEqComparable = requires(const T &t, const U &u) {
...
protected:
    ...
    const std::error_category *mErrorCatgory;
...
# define CO_ASYNC_EXPECTED_LOCATION \
     , const std::source_location &errLoc = std::source_location::current()
...
public:
    ...
    Expected(const std::error_code &ec CO_ASYNC_EXPECTED_LOCATION) noexcept
    ...
    Expected(const Expected &that) noexcept(
    ...
    Expected<std::invoke_result_t<F, const T &>> transform(F &&f) const & {
        if (has_value()) {
            return f(mValue), Void();
        }
        return {error() CO_ASYNC_EXPECTED_LOCATION_ARG};
    ...
    template <std::invocable F>
    Avoid<std::invoke_result_t<F, const T &>> and_then(F &&f) const & {
        if (has_value()) {
            return f(mValue), Void();
        }
        return *this;
    ...
    std::variant<std::reference_wrapper<const T>, std::error_code>
    repr() const {
        if (has_error()) {
            return std::error_code(mErrorCode, *mErrorCatgory);
        } else {
...
struct [[nodiscard("Expected<T> return values must be handled, use co_await to "
    ...
    Expected(const Expected<Void> &that) noexcept : Expected<Void>(that) {}

    Expected(Expected<Void> &&that) noexcept
        : Expected<Void>(std::move(that)) {}
};
