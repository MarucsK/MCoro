...
struct to_string_t<String> {
    ...
    void operator()(String &result,
                    const std::basic_string<char, Traits, Alloc> &value) const {
        result.assign(value);
    }
};
...
struct SplitString {
    ...
    /* Empty struct, serving as a range end marker. In range-based for,
     * end() returns a sentinel, which the iterator compares against to check
     * for the end. */
    struct sentinel {
    ...
    struct iterator {
        ...
        /* Remaining part starting from current substring. */
        ...
        friend bool operator==(const sentinel &lhs,
    ...
    private:
        ...
        std::string_view s; /* Remaining unprocessed string (processed parts are
                               removed from s). */
        Delim delimiter;
        std::string_view
            current;    /* Substring pointed to by current iterator. */
        bool ended;     /* Indicates if iteration has ended. */
        bool toBeEnded; /* Auxiliary flag for handling the last element. */
    };
