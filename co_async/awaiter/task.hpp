...
struct TaskAwaiter {
    ...
    TaskAwaiter(TaskAwaiter &&other) = delete;
...
struct TaskPromiseCommonBase {
    ...
    void *operator new(std::size_t size) {
        ...
        // void *ptr = std::pmr::get_default_resource()->allocate(size);
        // coroDb.mCoros.insert(ptr); // 添加：跟踪新分配的协程帧
        // std::cerr << "[DEBUG] CoroDb::new - Allocated frame at: " << ptr
        //           << ", size: " << size << std::endl;
        // return ptr;
    }
    ...
    void operator delete(void *ptr, std::size_t size) noexcept {
        ...
        // if (coroDb.mCoros.count(ptr)) {
        //     coroDb.mCoros.erase(ptr);
        //     std::cerr << "[DEBUG] CoroDb::delete - Deallocated frame at: "
        //               << ptr << ", size: " << size << std::endl;
        // } else {
        //     std::cerr << "[ERROR] CoroDb::delete - Attempt to deallocate "
        //                  "untracked frame at: "
        //               << ptr << ", size: " << size << std::endl;
        // }
        // std::pmr::get_default_resource()->deallocate(ptr, size);
...
struct TaskPromiseCommon : TaskPromiseCommonBase {
    ...
    const TaskPromise &self() const noexcept {
        return static_cast<const TaskPromise &>(*this);
    }
...
struct TaskPromiseExpectedTransforms {
    ...
    template <class T2, class E2> // E2 is unused but part of original
...
struct TaskPromiseTransforms {
    ...
    template <class U>
    const Task<U> &await_transform(const Task<U> &u) noexcept {
        u.promise().mLocals = self().mLocals;
        return u;
    }
...
struct TaskPromise : TaskPromiseImpl<TaskPromise<T>, T> {
    void return_value(T &&ret) {
        this->mAwaiter->returnValue(std::move(ret));
    }

    void return_value(const T &ret) {
        this->mAwaiter->returnValue(ret);
    }
#if CO_ASYNC_DEBUG
    void
    ...
    {
        this->mAwaiter->returnValue(std::forward<decltype(ret)>(ret), loc);
    }
    ...
    {
        this->mAwaiter->returnValue(std::forward<decltype(ret)>(ret));
    }
#else
    void return_value(std::convertible_to<T> auto &&ret) {
...
struct TaskPromise<GeneratorResult<T, E>>
    ...
    auto yield_value(T &&ret) {
        return this->mAwaiter->yieldValue(std::move(ret));
    }

    auto yield_value(const T &ret) {
        return this->mAwaiter->yieldValue(ret);
    }

    auto yield_value(std::convertible_to<T> auto &&ret) {
        return this->mAwaiter->yieldValue(std::forward<decltype(ret)>(ret));
    }
