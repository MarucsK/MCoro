...
struct ThreadPool::Thread {
    ...
    std::condition_variable mCV; // for waiting for new tasks.
    std::mutex mMutex;
    ThreadPool *mPool;
    std::jthread mThread;
    ...
    ~Thread() {
        ...
        mCV.notify_one(); // Wake up the task-waiting thread to check for a stop
                          // request.
        mThread.join();
    }
    ...
    void startMain(std::stop_token stop) {
        while (!stop.stop_requested()) [[likely]] {
            ...
            }); // Waiting for new tasks or stop requests.
    ...
    void workOn(std::function<void()> func) { // Assign tasks to threads.
