...
Task<int> hello1() {
    std::cout << "hello1 sleep for 1s" << std::endl;
    co_await sleep_for(1s);
    std::cout << "hello1 wake up" << std::endl;
    co_return 1;
}

Task<int> hello2() {
    std::cout << "hello2 sleep for 2s" << std::endl;
    co_await sleep_for(2s); // 2s 等价于 std::chrono::seconds(2)
    std::cout << "hello2 wake up" << std::endl;
    co_return 2;
}

Task<int> hello() {
    std::cout << "hello begin wait for 1 and 2" << std::endl;
    auto v = co_await when_any(hello1(), hello2(), hello2());
    /* co_await hello1(); */
    /* co_await hello2(); */
    std::cout << "hello see " << (int)v.index() + 1 << " wake up" << std::endl;
...
int main() {
    ...
    getLoop().addTask(t);
    getLoop().runAll();
    std::cout << "main get hello result: " << t.mCoroutine.promise().result()
              << std::endl;
    return 0;
}