...
auto checkError(auto res, const std::source_location &loc =
...
struct EpollLoop {
    ...
    void tryRun(int timeout) {
        int res = checkError(
            epoll_wait(mEpoll, mEventBuf, std::size(mEventBuf), timeout));
        for (int i = 0; i < res; i++) {
...
Marcus::Task<void> async_main() {
    int file =
        Marcus::checkError(open("CMakeLists.txt", O_RDONLY | O_NONBLOCK));
    while (true) {
