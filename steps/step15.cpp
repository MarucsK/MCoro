...
struct EpollFilePromise : Promise<std::uint32_t> {
...
struct EpollLoop {
    ...
    inline void
    tryRun(std::optional<std::chrono::system_clock::duration> timeout);
...
inline Task<uint32_t, EpollFilePromise> wait_file(EpollLoop &loop, int fileNo,
                                                  uint32_t events) {
...
Marcus::Task<std::string> reader(int fileNo) {
...
Marcus::Task<void> async_main() {
    int file = Marcus::checkError(open("/dev/stdin", O_RDONLY | O_NONBLOCK));
    while (true) {
        debug(), "begin to read";
        auto v = co_await when_any(reader(STDIN_FILENO), reader(file));
        std::string s;
        std::visit([&](const std::string &v) { s = v; }, v);
