...
template <typename T>
ReturnPreviousTask whenAllHelper(const auto &t, WhenAllCtlBlock &control,
...
Task<int> hello1() {
    std::cout << "hello1开始睡1秒" << std::endl;
    co_await sleep_for(1s);
    std::cout << "hello1睡醒了" << std::endl;
    co_return 1;
}

Task<int> hello2() {
    std::cout << "hello2开始睡2秒" << std::endl;
    co_await sleep_for(2s);
    std::cout << "hello2睡醒了" << std::endl;
    co_return 2;
}

Task<int> hello() {
    std::cout << "hello开始等1和2" << std::endl;
    auto v = co_await when_any(hello1(), hello2());
    /* co_await hello1(); */
    /* co_await hello2(); */
    std::cout << "hello看到 " << (int)v.index() + 1 << "睡醒了" << std::endl;
...
int main() {
    auto t = hello();
    getLoop().run(t);
    std::cout << "主函数中得到hello结果: " << t.mCoroutine.promise().result()
              << std::endl;
    return 0;
}
